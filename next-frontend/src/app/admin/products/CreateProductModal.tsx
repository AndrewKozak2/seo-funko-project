...
export function CreateProductModal({
  ...
  const [productMode, setProductMode] = useState<"standard" | "bundle">("standard");
    ...
    try {
      ...
      if (productMode === "standard") {
        ...
        if (selectedBundleItems.length !== 2) {
          toast.error("Please select exactly 2 products for the bundle!");
          setIsLoading(false);
          return;
        }
        if (!bundlePrice) {
          ...
          {productMode === "standard" && (
            <>
              ...
              <div className={styles.inputGroup}>
                ...
                <label className={styles.fileUploadArea}>
                  ...
                  <div className={styles.fileUploadPlaceholder}>
                    {imageFile ? <span>✓ {imageFile.name}</span> : <span>Click to upload an image</span>}
                  </div>
                </label>
              ...
              <div className={styles.inputGroup}>
                ...
                {isNewCollection ? (
                  ...
                  <div className={styles.customSelectWrapper}>
                    <div className={styles.customSelectTrigger} onClick={() => setIsDropdownOpen(!isDropdownOpen)}>
                    ...
                    {isDropdownOpen && (
                      <ul className={styles.customSelectList}>
                        {uniqueCollections.map((colName) => (
                          <li key={colName} className={styles.customSelectOption} onClick={() => {
              ...
              <div className={styles.checkboxGroup}>
                <label>
                  <input type="checkbox" checked={isNewCollection} onChange={(e) => {
              ...
              <div className={styles.checkboxGroup}>
                <label>
                  <input type="checkbox" checked={formData.isExclusive} onChange={(e) => setFormData({ ...formData, isExclusive: e.target.checked })} /> Is Exclusive?
                </label>
              </div>
            </>
          )}
            ...
            <>
              ...
              {bundleCollection && (
                <div className={styles.bundleSelectionArea}>
                  ...
                  {availableBundleProducts.length < 2 ? (
                    <p className={styles.warningText}>Not enough products in this collection.</p>
                  ) : (
                    <div className={styles.bundleGrid}>
                      {availableBundleProducts.map((product) => {
                        const isSelected = selectedBundleItems.includes(product.id);
                        return (
                          <div key={product.id} className={`${styles.bundleItem} ${isSelected ? styles.bundleItemSelected : ""}`} onClick={() => {
                            if (isSelected) {
              ...
              {selectedBundleItems.length === 2 && (
                <div className={styles.inputGroup}>
                  ...
                  <input type="number" placeholder="Enter bundle price" value={bundlePrice} onChange={(e) => setBundlePrice(e.target.value)} required />
                </div>
              )}
            </>
          )}
