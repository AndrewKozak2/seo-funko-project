...
export const useCheckout = () => {
  ...
  // 2. ОНОВЛЕННЯ: Змінна середовища для Next.js
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // 3. ЗАМІНА: Передаємо дані через URL, бо Success сторінка їх там чекає
      router.push(
        `/success?orderId=${savedOrder._id}&total=${finalPrice.toFixed(2)}`,
      );
